...
from error import can_findall, log
...
def zdnet():
  ...
  r = requests.get(url) #HTTPステータスによるハンドリング
  soup = BeautifulSoup(r.content, "html.parser")

  # リンク取得（リスト型：arc_lnklist）
  target = soup.find_all('a', {'data-ga_category': 'zd_top_list_new'}) # targetが空の場合
  if not target:
    log("ニュースリンクを取得できません",{"url":url,"status":r.status_code,"a_tag":soup.find_all('a')})
  ...
  for x in range(len(arc_lnklist)):
      ...
      # cont取得
      # 1ページ目
      cont_tags = arcsoup.find('div', class_="article-contents") #div.pがない場合
      if can_findall(cont_tags):
        cont_tags = cont_tags.find_all("p")
      else:
        log("div.pが存在しません。",{"url":url,"status":r.status_code,"cont_tags":cont_tags})
        continue
      ...
      for page_number in range(2, 10):
        ...
        if arcr_extra.status_code == 200:
          ...
          cont_tags = arcsoup_extra.find('div', class_="article-contents")
          if can_findall(cont_tags):#div.pがない場合
            cont_tags = cont_tags.find_all("p")
          else:
            log("div.pが存在しません。",{"url":url,"status":r.status_code,"cont_tags":cont_tags})
            continue
