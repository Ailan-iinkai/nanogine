...
url = "https://xtech.nikkei.com/top/latestdaily.html"
r = requests.get(url)
soup = BeautifulSoup(r.content, "html.parser")

# リンク取得（リスト型：arc_lnklist）
target = soup.find_all('a', class_="p-articleList_item_link")
arc_lnklist = [x["href"] for x in target]
...
# # arc_ttl,arc_lnklist,arc_contをネームドタプルに
# # namedtupleの定義
# zdarticle = namedtuple('zdarticle', ['title', 'context', 'url'])

# # 辞書にデータをまとめる
# data = {i: zdarticle(t, c, u) for i, (t, c, u) in enumerate(zip(arc_ttl, arc_cont, arc_lnklist), start=1)}
